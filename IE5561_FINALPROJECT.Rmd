...
for (pkg in c("naniar","forcats","rsample","dplyr")) {
...
print(table(df2$WEATHER_CONDITION))

X_mat <- model.matrix(
  ~ . - 1,
  data = df2 %>% select(all_of(cat_vars))
)

cat("Dummy matrix dims:", dim(X_mat), "\n")
...
# ── Section 3 (manual): Exploratory association analysis ────────────────────
...
assoc_df <- bind_rows(assoc_list) %>%
  arrange(desc(cramers_V))

print(head(assoc_df, 10))


# ── Section 4: Baseline classifiers (revised) ────────────────────────────────
...
test$glm_prob <- predict(glm_mod, test, type = "response")
...
# ── Section 5: Regularized logistic (glmnet) ─────────────────────────────────
...
cat("Non-zero @ lambda.min:", sum(coef_min != 0) - 1, "\n")
cat("Non-zero @ lambda.1se:", sum(coef_1se != 0) - 1, "\n")
...
pred_prob_lasso  <- predict(lasso_mod, newx = x_test, type = "response")[,1]
pred_class_lasso <- factor(
  ifelse(pred_prob_lasso > 0.5, "yes", "no"),
  levels = c("no","yes")
)
...
nm     <- sum(coef(cvfit, s="lambda.min") != 0) - 1
n1s    <- sum(coef(cvfit, s="lambda.1se") != 0) - 1
cat("Non-zero @ lambda.min:", nm, 
    "  @ lambda.1se:", n1s, "\n\n")
...
pred_class_def <- factor(ifelse(pred_prob_lasso > 0.5, "yes","no"),
                         levels=c("no","yes"))
cm_def <- confusionMatrix(pred_class_def,
                          factor(y_test, levels=c(0,1),labels=c("no","yes")),
                          positive="yes")
print(cm_def)

opt    <- coords(roc_lasso, "best", best.method="youden",
                 ret=c("threshold","sensitivity","specificity"))
print(opt)

thresh <- opt[1, "threshold"]
pred_class_opt <- factor(ifelse(pred_prob_lasso > thresh, "yes","no"),
                         levels=c("no","yes"))
cm_opt <- confusionMatrix(pred_class_opt, test$SevereFactor,
                          positive="yes")
cat("\nConfusion matrix at Youden threshold (", round(thresh,3), "):\n", sep = "")
print(cm_opt)


# ── Section 6: Tree-based learners (include speed) ───────────────────────────

for(pkg in c("rsample","caret","rpart","randomForest","gbm","pROC")) {
  if (!requireNamespace(pkg, quietly=TRUE)) install.packages(pkg)
  library(pkg, character.only=TRUE)
}
...
split_obj <- initial_split(df2, prop = 0.7, strata = "SevereInjury")
trn       <- training(split_obj)
tst       <- testing(split_obj)

trn$SevereFactor <- factor(trn$SevereInjury, levels = c(0,1), labels = c("no","yes"))
tst$SevereFactor <- factor(tst$SevereInjury, levels = c(0,1), labels = c("no","yes"))
...
# ── Section 7: Model comparison (ISL Ch 5) ──────────────────────────────────

library(pROC)

probs_cart  <- predict(cart_fit, newdata = tst, type = "prob")[, "yes"]
probs_bag   <- predict(bag_fit,  newdata = tst, type = "prob")[, "yes"]
probs_rf    <- predict(rf_fit,   newdata = tst, type = "prob")[, "yes"]
probs_gbm   <- predict(gbm_fit,  newdata = tst, type = "prob")[, "yes"]
probs_lasso <- pred_prob_lasso

roc_cart  <- roc(tst$SevereInjury, probs_cart)
roc_bag   <- roc(tst$SevereInjury, probs_bag)
roc_rf    <- roc(tst$SevereInjury, probs_rf)
roc_gbm   <- roc(tst$SevereInjury, probs_gbm)
roc_lasso <- roc(tst$SevereInjury, probs_lasso)
...
for(i in seq_along(mca_data)) {
  mca_data[[i]] <- as.factor(mca_data[[i]])
}
